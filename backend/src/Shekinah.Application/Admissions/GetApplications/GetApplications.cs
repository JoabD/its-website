...
public sealed record ApplicationListItem(string Id, string Folio, string ApplicantName, Modality Modality, string RegionName, ApplicationStatus Status, DateTime SubmittedAtUtc);
    ...
    : IQueryHandler<GetApplicationsQuery, PagedResult<ApplicationListItem>>
{
    public async Task<Result<PagedResult<ApplicationListItem>>> HandleAsync(GetApplicationsQuery query, CancellationToken ct)
    {
        ...
        var mapped = items.Select(a => new ApplicationListItem(
            a.Id, a.Folio, a.Applicant.FullName.FullName, a.ModalityChoice.Modality, a.ModalityChoice.RegionName, a.Status, a.SubmittedAtUtc)).ToList();
